...
public class Day03 extends Day {
    ...
    private static final String NOT_SYMBOL = "0123456789.";

    @Override
    protected void part1() {
        String[] lines = input.split("\n");
        List<NumberOnMap> numbers = getNumbersOnMap(lines);
        int code = numbers.stream()
                .filter(elem -> isLinked(elem, lines))
                .map(numberOnMap -> numberOnMap.value)
                .reduce(Integer::sum)
                .orElse(0);
        System.out.println("Part 1 : " + code);
    }

    private List<NumberOnMap> getNumbersOnMap(String[] lines) {
        List<NumberOnMap> numbers = new ArrayList<>();
        for (int i = 0; i < lines.length; i++) {
            String line = lines[i];
            boolean onNumber = false;
            int pos = 0;
            for (int j = 0; j < line.length(); j++) {
                boolean charIsNumber = NUMBERS.contains(line.charAt(j) + "");
                if (!onNumber && charIsNumber) {
                    onNumber = true;
                    pos = j;
                } else if (onNumber && !charIsNumber) {
                    onNumber = false;
                    String number = line.substring(pos, j);
                    numbers.add(new NumberOnMap(pos, i, number.length(), Integer.parseInt(number)));
                } else if (onNumber && j == lines.length - 1) {
    ...
    private boolean isLinked(NumberOnMap elem, String[] lines) {
        int mapWidth = lines[0].length();
        int mapHeigth = lines.length;
        boolean linked = false;

        if (elem.posX != 0) {
            linked = isLinkedLeft(elem, lines);
        }

        if (elem.posX + elem.length != mapWidth) {
            linked = linked || isLinkedRight(elem, lines);
        }

        if (elem.posY != 0) {
            linked = linked || isLinkedUp(elem, lines);
        }

        if (elem.posY + 1 != mapHeigth) {
            linked = linked || isLinkedDown(elem, lines);
        }

        if (elem.posX != 0 && elem.posY != 0) {
            linked = linked || isLinkedUpLeft(elem, lines);
        }

        if (elem.posX + elem.length != mapWidth && elem.posY != 0) {
            linked = linked || isLinkedUpRight(elem, lines);
        }

        if (elem.posX != 0 && elem.posY + 1 != mapHeigth) {
            linked = linked || isLinkedDownLeft(elem, lines);
        }

        if (elem.posX + elem.length != mapWidth && elem.posY + 1 != mapHeigth) {
            linked = linked || isLinkedDownRight(elem, lines);
        }

        return linked;
    }

    private boolean isLinkedDownRight(NumberOnMap elem, String[] lines) {
        return containsSymbol(lines[elem.posY + 1].charAt(elem.posX + elem.length) + "");
    }

    private boolean isLinkedDownLeft(NumberOnMap elem, String[] lines) {
        return containsSymbol(lines[elem.posY + 1].charAt(elem.posX - 1) + "");
    }

    private boolean isLinkedUpRight(NumberOnMap elem, String[] lines) {
        return containsSymbol(lines[elem.posY - 1].charAt(elem.posX + elem.length) + "");
    }

    private boolean isLinkedUpLeft(NumberOnMap elem, String[] lines) {
        return containsSymbol(lines[elem.posY - 1].charAt(elem.posX - 1) + "");
    }

    private boolean isLinkedDown(NumberOnMap elem, String[] lines) {
        return containsSymbol(lines[elem.posY + 1].substring(elem.posX, elem.posX + elem.length));
    }

    private boolean isLinkedUp(NumberOnMap elem, String[] lines) {
        return containsSymbol(lines[elem.posY - 1].substring(elem.posX, elem.posX + elem.length));
    }

    private boolean isLinkedRight(NumberOnMap elem, String[] lines) {
        return containsSymbol(lines[elem.posY].charAt(elem.posX + elem.length) + "");
    }

    private boolean isLinkedLeft(NumberOnMap elem, String[] lines) {
        return containsSymbol(lines[elem.posY].charAt(elem.posX - 1) + "");
    }

    private boolean containsSymbol(String str) {
        for (int i = 0; i < str.length(); i++) {
            if (NOT_SYMBOL.indexOf(str.charAt(i)) == -1) {
                return true;
            }
        }
        return false;
    ...
    private static class NumberOnMap {
        public int posX;
        public int posY;
        public int length;
        public int value;

        public NumberOnMap(int posX, int posY, int length, int value) {
            this.posX = posX;
            this.posY = posY;
            this.length = length;
            this.value = value;
